...
const makeMessagesSocket = (config) => {
    ...
    const getUSyncDevices = async (jids, ignoreZeroDevices) => {
        ...
        const extracted = Utils_1.extractDeviceJids(result, device, ignoreZeroDevices);
        const deviceMap = {};
    ...
    const relayMessage = async (jid, message, { messageId: msgId, additionalAttributes, cachedGroupMetadata }) => {
        ...
        if (isGroup) {
            ...
            for (const { user, device } of devices) {
                ...
                participants.push(await createParticipantNode(WABinary_1.jidEncode(user, 's.whatsapp.net', device), isMe ? encodedMeMsg : encodedMsg));
            }
            stanza = {
                tag: 'message',
                attrs: {
